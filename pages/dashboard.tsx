...
export default function Dashboard({ habits: initialHabits, tasks: initialTasks, dailyState: initialState, habitCheckins, taskCheckins }: DashboardProps) {
  ...
  const handleHabitCreate = async () => {
    ...
    const res = await fetch('/api/habits', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(habitForm) });
    if (res.ok) {
  ...
  const handleHabitDone = async (id: number) => {
    const res = await fetch('/api/checkins/habit', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ habitId: id }) });
    if (res.ok) {
  ...
  const handleTaskDone = async (id: number) => {
    const res = await fetch('/api/checkins/task', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ taskId: id }) });
    if (res.ok) {
  ...
  const handleDailyStateSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const res = await fetch('/api/daily-state', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
    if (res.ok) {
